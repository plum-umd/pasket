...
class Clazz(v.BaseNode):
  ...
  def __init__(self, **kwargs):
    ...
    # client (or platform)
    self._client = kwargs.get("client", False)
  ...
  @property
  def is_android(self):
    return self._pkg and self._pkg.startswith("android.")

  @property
  def is_gui(self):
    return self._pkg and (self._pkg.startswith("java.awt") or self._pkg.startswith("javax.swing"))
  ...
  @property
  def is_aux(self):
    return self._name.startswith("Aux")
  ...
  def merge(self, other):
    ...
    for fld_o in other.flds:
      ...
      if fld_me: fld_me.merge(fld_o)
      else:
        logging.debug("merging: {} -> {}".format(fld_o.name, self._name))
        self.add_fld(fld_o)

    # merge methods
    for mtd_o in other.mtds:
      mtd_me = self.mtd_by_sig(mtd_o.name, mtd_o.param_typs)
      if mtd_me: mtd_me.merge(mtd_o)
      else:
        logging.debug("merging: {} -> {}".format(mtd_o.name, self._name))
        self.add_mtd(mtd_o)
...
def find_fld(cname, fname, visited=[]):
  ...
  if (cls.is_aux or cls.is_itf) and cls.subs:
    for sub in cls.subs:
      if sub.name in visited: continue
      fld = find_fld(sub.name, fname, visited + [sub.name])
      if fld: return fld
  return None
