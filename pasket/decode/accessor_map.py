...
class AccessorMap(object):
  ...
  def simple_role_of_interest(msg):
    return re.match(AccessorMap.regex_role, msg)

  # add a mapping from role variable to its value chosen by sketch
  def add_simple_role(self, msg):
    m = re.match(AccessorMap.regex_role, msg)
    v, n = m.group(1), m.group(6)
    self._role[v] = n

  @staticmethod
  def gs_role_of_interest(msg):
    return re.match(AccessorMap.regex_gs_role, msg)

  @staticmethod
  def is_method_log(msg):
    return re.match(AccessorMap.regex_log, msg)
  
  def add_invoked(self, msg):
    m = re.match(AccessorMap.regex_log, msg)
    self._invoked.add(int(m.group(2)))
        
  # add a mapping from role variable to its value chosen by sketch
  def add_gs_role(self, msg):
    m = re.match(AccessorMap.regex_gs_role, msg)
  ...
  def __init__(self, output_path, acc_conf):
    ...
    with open(self._output, 'r') as f:
      for line in f:
        line = line.strip()
        try:
          if AccessorMap.is_method_log(line): self.add_invoked(line)
          items = line.split(',')
          func, kind, msg = items[0], items[1], ','.join(items[2:])
          if AccessorMap.simple_role_of_interest(msg): self.add_simple_role(msg)
          if AccessorMap.gs_role_of_interest(msg): self.add_gs_role(msg)
    ...
    for k in cons.iterkeys():
      ...
      if util.exists(lambda m: m.id in self._invoked, c.clazz.mtds):
        for n, t in enumerate(c.param_typs):
          AccessorMap.add_prvt_fld(c.clazz, k, t, n)
        AccessorMap.def_constructor(c, c.clazz)
