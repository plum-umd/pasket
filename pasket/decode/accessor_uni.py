...
class AccessorUni(object):
  ...
  regex_rcv = r"(rcv_{}\d+)".format(C.ACC.AUX+"Uni")

  @staticmethod
  def repl_rcv(msg):
    m = re.match(AccessorUni.regex_rcv, msg)
    if m: return msg.replace(m.group(0), C.J.THIS)
    else: return msg

  ## hole assignments for roles
  ## glblInit_subject_????,StmtAssign,subject_???? = n
  regex_role = r"(((({})_\S+)|({}))_{}\d+).* = (\d+)$".format('|'.join(C.acc_roles), '|'.join(C.adp_roles), C.ACC.AUX+"Uni")

  regex_gs_role = r"((({})_\S+_\d+)_{}\d+).* = (\d+)$".format('|'.join(C.acc_roles), C.ACC.AUX+"Uni")
